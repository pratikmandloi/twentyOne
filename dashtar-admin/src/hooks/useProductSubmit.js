...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const variantId = variantTitle.map((data) => data._id);
      ...
      const updatedVariants = variants.map((v, i) => {
        return {
          ...
          // attribute: variantTitle,
        };
      });

      console.log(updatedVariants, "car");
      ...
      if (updatedId) {
        ...
        if (isCombination) {
          setValue("title", res.product.title[language ? language : "en"]);
          setValue(
            "description",
            res.product.description[language ? language : "en"]
          );
          ...
          const result = res.product.variants.map(
            ({
              originalPrice,
              price,
              discount,
